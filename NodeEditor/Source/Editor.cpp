...
void ed::EditorContext::Begin(const char* id, const ImVec2& size)
{
	...
	ImGui::PushStyleColor(ImGuiCol_ChildWindowBg, (ImVec4)ImColor(0, 0, 0, 0));
...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control& control)
{
	...
	if (ImGui::IsWindowFocused() && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_X)) && Editor->AreShortcutsEnabled())
	{
...
ed::EditorAction::AcceptResult ed::ShortcutAction::Accept(const Control& control)
{
	...
	if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Z)))
